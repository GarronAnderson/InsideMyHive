...
with open(out_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['start date','end date', 'data source','R','t0','P2P raw','P2P corrected','P2P simple', 'P2P no t0', 'data points'])
    ...
    for start_time, end_time in date_pairs:
        ...
        if (scale_data.size > 10) and (temp_data.size > 10):      
            ...
            writer.writerow([str(start_time)[:10], str(end_time)[:10], "temp", best_r, best_t0, raw_ptp, corr_ptp, simple_ptp, t0_0_ptp, len(scale_data)])
    ...
    for start_time, end_time in date_pairs:
        if start_time > datetime(2024, 12, 17, 0, 0, 0):
            ...
            if (scale_data.size > 10) and (temp_data.size > 10):
                ...
                writer.writerow([str(start_time)[:10], str(end_time)[:10], "thermo", best_r, best_t0, raw_ptp, corr_ptp, simple_ptp, t0_0_ptp, len(scale_data)])
