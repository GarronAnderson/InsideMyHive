...
class Gauge:
    ...
    def render(self):
        outer_arc = Arc(self.radius, 260, 90, 25, arc_width=10, outline=0x0, fill=0xFFFFFF, x=self.x, y=self.y)
        ...
        alarm_arc = Arc(self.radius, alarm_ind_val, alarm_ind_angle, 25, arc_width=10, outline=0x0, fill=0xFFFFFF, x=self.x, y=self.y)
        
        alarming = self.val > self.max_alarm_val
        fill_color = 0xFF0000 if alarming else 0x000000
        
        inner_arc = Arc(self.radius, arc_val, arc_angle, 25, arc_width=10, outline=0x0, fill=fill_color, x=self.x, y=self.y)
        
        text_val = label.Label(terminalio.FONT, text=str(round(self.val,0)), color=0x0, scale=2)
...
class BatteryIndicator:
    def __init__(self, scale=2, x=0, y=0):
        ...
        self._points_scaled = [(((point[0]*self.scale)+self.x), (point[1]*self.scale)+self.y) for point in self._points]
